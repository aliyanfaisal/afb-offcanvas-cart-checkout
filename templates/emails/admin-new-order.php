...
if (!$plain_text) {
    echo '<p style="margin:0 0 16px; font-family: Helvetica, Arial, sans-serif;">' . esc_html__('A new order has been placed. Details below:', 'woocommerce') . '</p>';
}
...
// Custom items table rendering with sample wrappers and classes
if ($plain_text) {
    ...
    foreach ($order->get_items() as $item_id => $item) {
        ...
        // Left visual column (Product): Align Start ($text_align)
        ...
        // Middle visual column (Unit x Qty): -> Align End ($text_align_end)
        ...
        // Right visual column (Total): -> Align End ($text_align_end)
...
if ($plain_text) {
    ...
    if ($delivery_option === 'pickup' && $pickup_location) {
        ...
        // Standard addresses: 50% each
        // Align Start ($text_align)

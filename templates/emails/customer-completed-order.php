...
/**
 * Custom Customer Completed Order Email
 * Variables provided by WooCommerce:
 * - $order (WC_Order)
 * - $email_heading
 * - $sent_to_admin
 * - $plain_text
...
// Intro text (optional)
if (!$plain_text) {
    echo '<p style="margin:0 0 16px; font-family: Helvetica, Arial, sans-serif;">' . esc_html__('Your order has been completed. Here are the details:', 'woocommerce') . '</p>';
}

// Order details (items table etc.)
...
// Custom items table rendering (matches sample structure)
if ($plain_text) {
    foreach ($order->get_items() as $item_id => $item) {
        ...
        // Render recipient details when multiship via existing hook (plain text)
    ...
    foreach ($order->get_items() as $item_id => $item) {
        ...
        // Left column (Product): Align Start ($text_align)
        echo '<td class="td font-family text-align-' . $text_align . '" style="color: #4b4b4b; border-bottom: 1px solid rgba(0, 0, 0, .2); font-family: Helvetica, Arial, sans-serif; text-align: ' . $text_align . '; padding: 8px 12px; padding-' . $text_align . ': 0; vertical-align: middle; word-wrap: break-word;" align="' . $text_align . '">';
        echo '<table class="order-item-data" style="color: #1e1e1e; font-family: Helvetica, Arial, sans-serif;">';
        echo '<tbody><tr>';
        echo '<td style="vertical-align: middle; padding-' . $text_align . ': 0; border-bottom: 1px solid rgba(0,0,0,.2); padding-bottom: 24px; border-bottom: 1px solid rgba(0, 0, 0, .2); padding: 0;">';
        if ($thumb_url !== '') {
            echo '<img width="48" height="48" src="' . esc_url($thumb_url) . '" class="attachment-48x48 size-48x48 skip-lazy no-lazyload lazyload-disabled no-lazy skip-lazyload" alt="' . esc_attr($alt_text) . '" data-skip-lazy="true" style="border: none; display: inline-block; font-size: 14px; font-weight: bold; height: auto; outline: none; text-decoration: none; text-transform: capitalize; vertical-align: middle; margin-' . $text_align_end . ': 24px; max-width: 100%;" border="0">';
        }
        echo '</td>';
        echo '<td style="vertical-align: middle; padding-' . $text_align_end . ': 0; border-bottom: 1px solid rgba(0,0,0,.2); padding-bottom: 24px; border-bottom: 1px solid rgba(0, 0, 0, .2); padding: 0;">';
        echo '<strong>' . esc_html($name) . '</strong>';
        echo '<div class="email-order-item-meta" style="color: #b7c4ca; font-size: 14px; line-height: 140%; font-family: Helvetica, Arial, sans-serif;"></div>';
        // Recipient address/phone/message
        ob_start();
        ...
        // Middle column (Unit x Qty): Align End ($text_align_end)
        ...
        // Right column (Total): Align End ($text_align_end)
    ...
    if (is_array($totals) && !empty($totals)) {
        ...
        foreach ($totals as $key => $total) {
            ...
            // Value: Align End ($text_align_end)
...
// After items: conditional delivery sections
...
if ($delivery_option === 'multiship') {
    // Item-level recipient address/phone/message are rendered by existing hooks
    if (!$plain_text) {
        // echo '<p style="margin:16px 0 0; color:#666; font-size:12px;">' . esc_html__('Note: Shipping details are hidden for multi-ship orders. Recipient info appears under each item.', 'afb-offcanvas') . '</p>';
    }
} elseif ($delivery_option !== 'pickup') {
    // Other delivery option: show a simple section
    $label_map = [
...
if ($plain_text) {
    ...
    if ($delivery_option === 'pickup' && $pickup_location) {
        // Pickup Logic: Align Start ($text_align)
        echo '<td colspan="2" valign="top" align="' . $text_align . '" style="font-family: Helvetica, Arial, sans-serif; text-align:' . $text_align . '; border:0; padding:0">';
        ...
        // Standard: Align Start ($text_align)
        echo '<td valign="top" width="50%" align="' . $text_align . '" style="font-family: Helvetica, Arial, sans-serif; text-align:' . $text_align . '; border:0; padding:0">';
        echo '<b style="color:#1e1e1e; font-family: Helvetica, Arial, sans-serif">' . esc_html(__('Billing address', 'woocommerce')) . '</b>';
        echo '<address style="color:#1e1e1e; font-style:normal; padding:8px 0; word-break:break-all">';
        echo wp_kses_post($billing_address_html);
        ...
        echo '</address>';
        echo '</td>';
        echo '<td valign="top" width="50%" align="' . $text_align . '" style="font-family: Helvetica, Arial, sans-serif; text-align:' . $text_align . '; padding:0; padding-' . $text_align_end . ':10px">';

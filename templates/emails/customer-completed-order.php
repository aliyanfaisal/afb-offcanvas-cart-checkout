...
// Intro text (optional)
if (!$plain_text) {
    echo '<p style="margin:0 0 16px; font-family: \"Helvetica Neue\", Helvetica, Roboto, Arial, sans-serif;">' . esc_html__('Your order has been completed. Here are the details:', 'woocommerce') . '</p>';
}
...
if ($plain_text) {
    ...
    foreach ($order->get_items() as $item_id => $item) {
        ...
        // Left column: product title and meta block
        echo '<td class="td font-family text-align-left" style="color: #4b4b4b; border-bottom: 1px solid rgba(0, 0, 0, .2); font-family: \"Helvetica Neue\",Helvetica,Roboto,Arial,sans-serif; text-align: left; padding: 8px 12px; padding-left: 0; vertical-align: middle; word-wrap: break-word;" align="left">';
        echo '<table class="order-item-data" style="color: #1e1e1e; font-family: \"Helvetica Neue\",Helvetica,Roboto,Arial,sans-serif;">';
        echo '<tbody><tr>';
        echo '<td style="vertical-align: middle; padding-left: 0; border-bottom: 1px solid rgba(0,0,0,.2); padding-bottom: 24px; border-bottom: 1px solid rgba(0, 0, 0, .2); padding: 0;">';
        if ($thumb_url !== '') {
            echo '<img width="48" height="48" src="' . esc_url($thumb_url) . '" class="attachment-48x48 size-48x48 skip-lazy no-lazyload lazyload-disabled no-lazy skip-lazyload" alt="' . esc_attr($alt_text) . '" data-skip-lazy="true" style="border: none; display: inline-block; font-size: 14px; font-weight: bold; height: auto; outline: none; text-decoration: none; text-transform: capitalize; vertical-align: middle; margin-right: 24px; max-width: 100%;" border="0">';
        }
        echo '</td>';
        echo '<td style="vertical-align: middle; padding-right: 0; border-bottom: 1px solid rgba(0,0,0,.2); padding-bottom: 24px; border-bottom: 1px solid rgba(0, 0, 0, .2); padding: 0;">';
        echo '<strong>' . esc_html($name) . '</strong>';
        echo '<div class="email-order-item-meta" style="color: #b7c4ca; font-size: 14px; line-height: 140%; font-family: \"Helvetica Neue\", Helvetica, Roboto, Arial, sans-serif;"></div>';
        // Recipient address/phone/message: use existing hooks, shown only for multiship
        ob_start();
        ...
        // Middle column: unit price × quantity
    ...
    // Render totals similar to Woo template
